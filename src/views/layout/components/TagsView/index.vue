  ...
  setup() {
    ...
    const state = reactive({
      ...
      closeOthersTags: () => {
        ...
        store.dispatch('DelOtherView', state.selectedTag as TagView)
      },
    ...
    const visitedViews = computed(() => {
      return store.state.visitedViews
    })
    const routes = computed(() => store.state.permission.menus)
    ...
    const initTags = () => {
      ...
      for (const tag of state.affixTags) {
        // Must have tag name
        if (tag.name) {
          store.dispatch('AddVisitedView', tag as TagView)
        }
      }
    }
    ...
    const addTags = () => {
      if (currentRoute.name) {
        ...
        store.dispatch('AddView', currentRoute)
      }
      return false
    }
        ...
        for (const tag of tags) {
          if ((tag.to as TagView).path === currentRoute.path) {
            ...
            if ((tag.to as TagView).fullPath !== currentRoute.fullPath) {
              store.dispatch('UpdateVisitedView', currentRoute)
            }
          }
        }
      })
